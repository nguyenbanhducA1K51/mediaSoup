    ...
    try {
      if (router.canConsume({
        ...
        const plainTransport = await router.createPlainTransport({
          listenIp: '0.0.0.0', // Địa chỉ IP để lắng nghe dữ liệu từ producer
          rtcpMux: true, // Sử dụng RTCP Mux
          comedia: true // Sử dụng COMEDIA
        });

        consumer2 = await plainTransport.consume({
          producerId: producer.id,
          rtpCapabilities,
          paused: true, 
        });
        ...
        await consumer2.enableTraceEvent([ "rtp", "pli" ]);
